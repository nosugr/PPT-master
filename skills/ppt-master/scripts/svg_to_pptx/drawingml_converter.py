...
_NON_VISUAL_TAGS = frozenset((
    ...
    # Gradient and filter definition elements that LLMs sometimes place
    # outside <defs> at the SVG root level. They are not visual shapes.
    'linearGradient', 'radialGradient', 'filter', 'pattern',
    'clipPath', 'mask', 'symbol', 'marker',
    'stop', 'feGaussianBlur', 'feOffset', 'feBlend', 'feComposite',
    'feColorMatrix', 'feMerge', 'feMergeNode', 'feFlood', 'feTile',
    'feTurbulence', 'feDisplacementMap', 'feImage', 'feConvolveMatrix',
    'feDiffuseLighting', 'feSpecularLighting', 'fePointLight',
    'feDistantLight', 'feSpotLight', 'feComponentTransfer',
    'feFuncR', 'feFuncG', 'feFuncB', 'feFuncA',
    'animate', 'animateTransform', 'animateMotion', 'set',
    'script', 'view', 'cursor', 'font', 'font-face',
))
...
def _convert_use(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    """Handle SVG <use> elements.

    Two cases:
    1. ``<use data-icon="...">`` — icon placeholder that use_expander failed to
       expand (icon file missing, parse error, etc.). Silently skip so the rest
       of the slide still converts.
    2. ``<use href="#id">`` / ``<use xlink:href="#id">`` — standard SVG symbol
       reference. Look up the referenced element in ctx.defs and convert it as
       if it were inlined, applying the <use> element's x/y/width/height as an
       additional translate transform.
    """
    import copy

    # Case 1: unresolved data-icon placeholder — skip silently
    if elem.get('data-icon'):
        return None

    # Case 2: standard href reference
    XLINK_NS = 'http://www.w3.org/1999/xlink'
    href = elem.get('href') or elem.get(f'{{{XLINK_NS}}}href') or ''
    if not href.startswith('#'):
        return None  # external reference — skip

    ref_id = href[1:]
    referenced = ctx.defs.get(ref_id)
    if referenced is None:
        return None  # dangling reference — skip

    # Clone the referenced element so we don't mutate the defs
    clone = copy.deepcopy(referenced)

    # Wrap in a <g> with translate so existing group handling picks up the offset
    ...
    # Inherit presentation attributes from the <use> element
...
def collect_defs(root: ET.Element) -> dict[str, ET.Element]:
    ...
    Also picks up gradient/filter/pattern elements placed directly at the SVG
    root level (outside <defs>), which some LLM-generated SVGs do incorrectly
    ...
    for defs_elem in root.iter('defs'):
        for child in defs_elem:
            ...
            if elem_id:
                defs[elem_id] = child
    # Pick up root-level definition elements placed outside <defs>
    _DEF_LIKE_TAGS = frozenset((
        'linearGradient', 'radialGradient', 'filter', 'pattern',
        'clipPath', 'mask', 'symbol', 'marker',
    ))
    for child in root:
        local = child.tag.split('}', 1)[-1] if '}' in child.tag else child.tag
        if local in _DEF_LIKE_TAGS:
            elem_id = child.get('id')
            if elem_id:
                defs[elem_id] = child
    return defs
