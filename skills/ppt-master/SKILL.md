...
### Step 3: Template Selection (MANDATORY — cannot be skipped)

🚧 **GATE**: Step 2 complete; project directory structure is ready.

⛔ **BLOCKING**: Template selection requires explicit user confirmation. AI MUST wait for the user's decision before proceeding to Step 4.

**Decision tree**:

| User input | Step 3 action |
|---|---|
| An explicit path to a template directory (e.g. `skills/ppt-master/templates/layouts/academic_defense/`, `projects/foo/template/`) | Copy that directory's SVGs + `design_spec.md` + assets into the project, confirm with user, advance |
| A bare template name ("用 academic_defense 模板", "招商银行风格") | Resolve to `skills/ppt-master/templates/layouts/<name>/`, present the match to user for confirmation, then copy |
| No template specified / style description / vague intent / silence | **Recommend & ask** — see below |

**Recommend & ask flow** (when user did not specify a template):

1. Read `templates/layouts/layouts_index.json` to load the template library
2. Analyze the source content (from Step 1) and any style hints in the user's request
3. Recommend **1–2 templates** that best fit, presenting each with:
   - Template name and path
   - Summary (from `layouts_index.json`)
   - Keywords
   - Brief reasoning for the recommendation
4. Present the recommendation as a choice:
   - Use the recommended template
   - Pick a different template from the full library
   - Free design (no template)
5. ⛔ **Wait for explicit user response** before proceeding

> Style descriptions ("麦肯锡风格" / "Keynote 风" / "极简风" / etc.) are used as input for the recommendation — the AI matches them against template keywords and summaries. They also flow into Strategist's Eight Confirmations as a style brief.

> "What templates exist?" — list entries from `layouts_index.json` together with their paths, then still ask the user to choose.

> To create a new template, read `workflows/create-template.md`.

**Copy command** (once template is confirmed):

```bash
TEMPLATE_DIR=<confirmed template path>
...
**Page Structure Proposal** (after Eight Confirmations, before Design Spec):

⛔ **BLOCKING**: after the user confirms the Eight Confirmations, AI MUST read the source materials in detail, then propose a **page-by-page content outline** and wait for explicit user confirmation or modification before writing `design_spec.md`.

Requirements for the page structure:
- List every page with: **page number**, **page type** (cover / table of contents / content / summary / closing, etc.), and **specific content** for that page
- "Specific content" means the actual substance extracted from source materials — names of methods, data points, conclusions, case names, formulas, arguments, etc. Generic labels like "introduce background" or "analyze results" are NOT acceptable
- The outline must reflect the logical flow of the source material: which topics come first, how they build on each other, where transitions happen
- Present as a numbered list so the user can reference pages by number when requesting changes

> If the user requests changes (e.g., "merge pages 5 and 6", "add a page about X between 3 and 4"), revise the outline and re-present until the user confirms.

> Once the page structure is confirmed, it becomes the authoritative content outline for `design_spec.md` §IX and drives the `spec_lock.md` page entries.
...
- [x] Page Structure Proposal confirmed (user approved detailed page-by-page outline)
