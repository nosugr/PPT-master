...
> **Minimum font size constraint** (PPT readability floor):
> - Body text: minimum **12pt** (≈16px). Never go below this — text becomes unreadable in presentation mode.
> - Subtitle / sub-heading: minimum **14pt** (≈19px).
> - All higher-level headings (page title, chapter title, cover title) must be strictly larger than subtitle.
> - These are hard floors — the ramp table's ratio bands still apply above them.
