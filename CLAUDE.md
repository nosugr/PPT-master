...
- **八项确认附带预览图**: 每次输出八项确认表格后，从项目中选择与推荐风格最接近的一张图片，输出其路径和简要描述，供用户确认。优先从 `docs/assets/screenshots/` 查找，无匹配则在整个项目内搜索。
- **导出路径**: 使用 `-o exports/<子文件夹>/<文件名>.pptx` 直接导出到按用途分组的子文件夹，不生成 backup/ 目录。子文件夹名用英文加下划线（如 `ppt_master_guide`），子文件夹内的 .pptx 文件用对应的中文命名（如 `项目使用流程.pptx`）。命名由 AI 根据用户描述自动总结，不要使用无意义的默认名称。同时跳过 legacy SVG 版 PPTX 的生成。
